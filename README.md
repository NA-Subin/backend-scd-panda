...
> 📄 อยากได้เนื้อหาเดียวกันนี้แบบอ่านง่ายกว่า (มีลิงก์ สารบัญ เช็กลิสต์ติ๊กได้ และมีคำอธิบายภาพรวมระบบแบบ
> ไม่ต้องมีพื้นฐานเทคนิคมาก่อนก็อ่านเข้าใจ) เปิดไฟล์ **[`DEPLOY_GUIDE.html`](./DEPLOY_GUIDE.html)** ในโฟลเดอร์นี้
> ด้วยเบราว์เซอร์ได้เลย — ไฟล์นี้ตั้งใจเก็บไว้ใช้เองตอนส่งมอบเท่านั้น **ไม่ได้ถูก deploy ขึ้นเว็บจริง**
> (ไม่ได้อยู่ใน `public/` ของ frontend) — **ถ้าผู้รับช่วงต่อไม่ได้เขียนโค้ดมาก่อน แนะนำให้อ่านไฟล์นี้แทน README**

> ⚠️ **repo นี้มีแค่โค้ด ไม่มีข้อมูลจริงของบริษัทอยู่เลย** เอาโค้ดไปติดตั้งอย่างเดียวจะได้ระบบที่ว่างเปล่า
> ต้องมีไฟล์ข้อมูล (`.sql` backup หรือ Firebase export) ส่งมอบมาแยกต่างหากด้วยเสมอ — ดูรายละเอียดที่
> "ขั้นตอนที่ 5" ด้านล่าง หรือหัวข้อ "ภาพรวมระบบ" ใน `DEPLOY_GUIDE.html`
...
ระบบต่อฐานข้อมูลโดยกำหนด `search_path` ไปที่ **schema `public`** (schema เริ่มต้นของ Postgres) ต้องสร้าง
แค่ฐานข้อมูลเปล่าไว้ก่อนเริ่มใช้งาน:
...
> ⚠️ **อย่าสร้าง schema ชื่อ `scd_panda` แยกต่างหากแล้วใช้ผสมกับ `public`** — เคยมีการตั้งค่าแบบนั้นในช่วงแรก
> ของโปรเจกต์ (ดูค่า `PGSCHEMA` เก่าใน git history) แต่ระบบปัจจุบันต่อ `public` เท่านั้น schema เก่าที่ค้างอยู่
> (ถ้ามี) จะไม่ถูกแตะต้องเลย ทำให้ดูเหมือนมี "ฐานข้อมูลสำรอง" ซ่อนอยู่ทั้งที่จริง ๆ เป็นแค่ขยะเก่า สร้างความสับสน
> ตอน debug ได้มาก ถ้าเจอ schema แปลก ๆ แบบนี้ในฐานข้อมูลที่รับช่วงต่อมา ให้ตรวจสอบก่อนว่ามีข้อมูลสำคัญค้างอยู่
> ไหมก่อนลบทิ้ง
...
GRANT ALL ON SCHEMA public TO scd_panda_app;
GRANT ALL ON ALL TABLES IN SCHEMA public TO scd_panda_app;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO scd_panda_app;
```
...
- **สร้างตำแหน่งใหม่ (`POST /api/positions`) ต้องมีสิทธิ์ผู้ดูแลระบบ** เหมือนกับการแก้ไขตำแหน่งเดิม (`PUT`) —
  ไม่งั้นผู้ใช้ธรรมดาจะสร้างตำแหน่งที่มีสิทธิ์สูง (เช่น `FinancialData`) แล้วสมัครบัญชีใหม่ผูกกับตำแหน่งนั้นเอง
  ผ่าน `/api/auth/register` ได้ (ดู `ADMIN_ONLY_CREATE_TABLES` ใน `src/routes/tables.js`)

### พฤติกรรมการนำเข้าข้อมูล (import) ที่ควรรู้

การนำเข้าข้อมูล (`buildImportPlan` ใน `src/importData.js`, ใช้ทั้งปุ่ม "นำเข้าข้อมูล JSON" และ
`/api/admin/import`) ออกแบบให้จบด้วย **"success" อย่างเดียวในสถานการณ์ปกติ** ไม่ควรเห็นคำเตือนถ้าไฟล์ข้อมูล
ไม่มีปัญหาจริง ๆ — ถ้าเจอคำเตือนซ้ำ ๆ ทุกครั้งที่ import ให้ตรวจสอบตามนี้ก่อนสงสัยว่าโค้ดมีบั๊ก:

- **ค่า id ซ้ำกันในตารางเดียวกัน** (เช่น `companypayment`) จะถูก**แก้ไขอัตโนมัติ** ไม่ใช่แค่เตือน — แถวที่
  Firebase สร้างก่อนจะได้ id เดิม ส่วนแถวที่ชนทีหลังจะได้ id ใหม่ที่ไม่ซ้ำใคร (ดู `deduplicateFkTargetIds`) ใช้
  กฎนี้เฉพาะตารางที่มีตารางอื่นอ้างอิงถึงจริงเท่านั้น (`FK_TARGET_TABLES`) — `order`/`tickets` มี `id` ซ้ำได้
  โดยธรรมชาติ (เป็นเลขตั๋ว/ทริปที่วนซ้ำ ไม่ใช่คีย์) และไม่มีตารางไหนอ้างอิงเลย จึงไม่ถูกแตะต้อง
- ค่า `"0:ไม่มี"`/`"0:ว่าง"` (และ `TicketName` ที่เป็น `"1:ตั๋วเปล่า"`) เป็น placeholder ของแอปเดิมที่แปลว่า
  "ยังไม่ได้เลือก" ไม่ใช่ข้อมูลเสีย ไม่ถูกนับเป็น "เชื่อมข้อมูลไม่สำเร็จ"
- `company_history` และ `customer` **ไม่ได้มาจาก Firebase เลย** (ดู `NON_FIREBASE_TABLES`) จึงไม่ถูกนับเป็น
  "ไฟล์นี้ไม่มีตาราง" — `company_history` ถูกสร้างใหม่จาก `company.history` (JSONB) อัตโนมัติทุกครั้งที่ import
  เต็มรูปแบบ (จำเป็นเพราะมี FK ไปที่ `company` ซึ่งถูก DROP TABLE ใหม่ทุกครั้ง ถ้าไม่ rebuild จะหายไปเงียบ ๆ)
  ส่วน `customer` ถูกสร้างแบบ `IF NOT EXISTS` ทุกครั้งเช่นกัน — **ไม่ต้องรัน
  `sql/create_company_history_table.sql`/`sql/create_customer_table.sql` เองอีกต่อไป** (สคริปต์ทั้งสองไฟล์นี้
  เก็บไว้อ่านอ้างอิงเฉยๆ ไม่จำเป็นต้องรันแล้ว)
- ข้อมูลต้นทาง (Firebase) บางจุดกรอกขาดเป็นรายแถว (เช่น ลืมเลือกประเภทลูกค้า) ที่ไม่มีรูปแบบทั่วไปให้เดาได้ —
  จุดที่เจอแล้วและยืนยันคำตอบถูกต้องแล้วถูกฝังไว้เป็น `KNOWN_DATA_CORRECTIONS` ใน `src/importData.js` ทำงาน
  อัตโนมัติทุกครั้ง (เช็คก่อนว่าแถวยังตรงกับที่ตรวจไว้ไหม ถ้า Firebase ถูกแก้แล้วจะข้ามเองอัตโนมัติ) ถ้าเจอปัญหา
  แบบเดียวกันเพิ่มในอนาคต (ข้อมูลจุดเดียวที่เดาไม่ได้จากรูปแบบทั่วไป) ให้เพิ่มรายการใหม่ในนั้นได้เลย
